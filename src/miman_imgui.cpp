...
extern CFE_EVS_LongEventTlm_Payload_t *cFS_Event;
...
static const int INITIAL_CMD_INDICES[] = {
    ...
    72, 73, 74, 76, 77, 79, 81, 82, 85, 86, 58, // ADCS sunpointing용
    155,    // Enable Beacon
    200,
...
void ImGui_BeaconWindow(float fontscale)
{
    ...
    //                 ImGui::TableNextRow();
    //                 ImGui::TableSetColumnIndex(0);
    //                 ImGui::TextUnformatted("[CCSDS]");
    //                 ImGui::TableSetColumnIndex(1);
    //                 ImGui::TextUnformatted("");

    //                 ReportRow("CCMessage_ID", "0x%04X", g_last_report.CCMessage_ID);
    //                 ReportRow("CCCount",      "0x%04X", g_last_report.CCCount);
    //                 ReportRow("CCLength",     "0x%04X", g_last_report.CCLength);
    ...
    //                             g_last_report.CCTime_code[0], g_last_report.CCTime_code[1],
    //                             g_last_report.CCTime_code[2], g_last_report.CCTime_code[3],
    //                             g_last_report.CCTime_code[4], g_last_report.CCTime_code[5]);
    ...
    //                 ReportRow("msg_id",       "0x%04X", g_last_report.msg_id);
    //                 ReportRow("cc",           "%u",     g_last_report.cc);
    //                 ReportRow("ret_type",     "0x%02X", g_last_report.ret_type);
    //                 ReportRow("ret_code",     "%d",     g_last_report.ret_code);
    //                 ReportRow("ret_val_size", "%u",     g_last_report.ret_val_size);
    ...
    //                 ImGui::TableSetColumnIndex(1);
    //                 ImGui::Text("%d bytes", (int)g_last_report.payload.size());
    //             }
    ...
    //         if (g_last_report.valid && !g_last_report.payload.empty())
    //         {
    //             ImGui::Separator();
    ...
    //             for (int i = 0; i < (int)g_last_report.payload.size(); i++)
    //             {
    //                 ImGui::SameLine(0.0f, 0.0f);
    //                 ImGui::Text("%02X ", g_last_report.payload[i]);
        ...
        if (ImGui::BeginTabItem("Report"))
        {
                                        ...
                                        ImGui::GetContentRegionAvail().y * 0.4f)))
            {
                ...
                else
                {
                    ...
                    ReportRow("CCMessage_ID", "0x%04X", (unsigned int)g_report_view.CCMessage_ID);
                    ReportRow("CCCount",      "0x%04X", (unsigned int)g_report_view.CCCount);
                    ReportRow("CCLength",     "0x%04X", (unsigned int)g_report_view.CCLength);
                    ...
                    ImGui::TableSetColumnIndex(1);
                    ImGui::Text("%02X %02X %02X %02X %02X %02X",
                                g_report_view.CCTime_code[0], g_report_view.CCTime_code[1],
                                g_report_view.CCTime_code[2], g_report_view.CCTime_code[3],
                                g_report_view.CCTime_code[4], g_report_view.CCTime_code[5]);
                    ...
                    ReportRow("msg_id",       "0x%04X", (unsigned int)g_report_view.msg_id);
                    ReportRow("cc",           "%u",     (unsigned int)g_report_view.cc);
                    ReportRow("ret_type",     "0x%02X", (unsigned int)g_report_view.ret_type);
                    ReportRow("ret_code",     "%d",     (int)g_report_view.ret_code);
                    ReportRow("ret_val_size", "%u",     (unsigned int)g_report_view.ret_val_size);
                    ...
                    {
                        auto &pl = g_report_view.u.adcs_unsolicited_tlm_tlm;
                        ...
                        ImGui::Separator();
                        auto PrintBitmask = [&](const char *title, uint8_t mask[5])
                        {
                            ImGui::TableNextRow();
                            ImGui::TableSetColumnIndex(0);
                            ImGui::TextUnformatted(title);
                            ...
                            {
                                uint8_t b = mask[i];
                                ImGui::Text("idx %d: 0x%02X (byte)", i, b);
                                ImGui::Text("       b7 b6 b5 b4 b3 b2 b1 b0 = %d%d%d%d%d%d%d%d",
                                            (b>>7)&1, (b>>6)&1, (b>>5)&1, (b>>4)&1,
                                            (b>>3)&1, (b>>2)&1, (b>>1)&1, (b>>0)&1);
                            }
                ...
                {
                    auto &pl = g_report_view.u.uant_getstatus;
                    ...
                    default:
                        ImGui::Text("Generic payload (%u bytes)", g_report_view.ret_val_size);
                        break;
                    }
                ...
                (g_report_view.kind == REPORT_KIND_SC_GENERIC))
            {
                ...
                int bytes_per_line = 16;
                uint16_t dump_size = g_report_view.ret_val_size;
                if (dump_size > sizeof(g_report_view.u.generic.bytes))
                    dump_size = sizeof(g_report_view.u.generic.bytes);

                for (int i = 0; i < (int)dump_size; i++)
                {
                    ImGui::SameLine(0.0f, 0.0f);
                    ImGui::Text("%02X ", g_report_view.u.generic.bytes[i]);
                    if ((i + 1) % bytes_per_line == 0)
                        ImGui::NewLine();
                }
    ...
    if(ImGui::BeginTabItem("Event")) {
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.650f, 0.266f, 0.322f, 1.0f));
        ImGui::Text("App Name : %s", cFS_Event->PacketID.AppName);
        ImGui::Text("Event ID : %u", cFS_Event->PacketID.EventID);
        ImGui::Text("Event Type : %u", cFS_Event->PacketID.EventType);
        ImGui::Text("S/C ID : %u", cFS_Event->PacketID.SpacecraftID);
        ImGui::Text("Proc ID : %u", cFS_Event->PacketID.ProcessorID);
        ImGui::Text("Message : %s", cFS_Event->Message);
        ImGui::PopStyleColor();
    ...
    // }
        ...
        case 77: {  
            ...
            if (!initialized) {
                ...
                // ==== HSS0..1 ====
                p.HSS0_alpha = p.HSS0_beta = p.HSS0_gamma = 0;
                p.HSS1_alpha = p.HSS1_beta = p.HSS1_gamma = 0;

                // ==== MAG0..1 ====
                p.MAG0_alpha = 0; p.MAG0_beta = -90; p.MAG0_gamma = 0;
        ...
        case 85: { 
            ...
            ImGui::Text("MAG0 Sensing Element : %s", p.Mag0SensingElement ? "ENABLED" : "DISABLED");
            ImGui::Text("MAG1 Sensing Element : %s", p.Mag1SensingElement ? "ENABLED" : "DISABLED");
...
case 155: { // TO_LAB Add Packet
    ...
    break;
        ...
        case 159: { // TO_LAB Output Enable
            ...
            auto &pl = command->tolabenableoutputcmd.Payload;

            ImGui::Separator();
            ImGui::Text("TO_LAB_EnableOutput Payload");
            ImGui::InputText("dest_IP", pl.dest_IP, sizeof(pl.dest_IP));
            ...
            ImGui::Text("dest_IP: %s", pl.dest_IP);
            break;
        }
    ...
    if (ImGui::Button("Generate CMD")) {
        ...
        case 161: { // BEE-1000 UANT uantburnchannelcmd
            ...
            if (ImGui::Button("Generate CMD")) {
                ...
                // CRC 계산 (헤더+페이로드 전체, CRC 바이트는 0으로 놓고 시작)
 ...
 //8192, 0, 15
    ...
    case 199: { // ADDED FOR FTP TEST 1210, es start app cmd
        ...
        static char app_buf[20] = "MOZART";
        static char entry_buf[20] = "MOZART_Main";
        static char filename_buf[64] = "/cf/mozart_strip.so";

        static uint32_t StackSize = 8192;
        static uint8_t  ExceptionAction = 0;
        static uint16_t Priority = 90;
    ...
    case 200: { // ADDED FOR FTP TEST 1210
        /* 1211 Revise default buffer for payuzuc (KissCAM) app reload */
        static uint16_t msgid = 0x1800;
        static uint8_t fnccode = 7;
        static char app_buf[20] = "PAYUZUC";
        static char path_buf[64] = "/cf/payuzuc23.so";
...
case 205: {
    ...
    case 207: { // PAYUZUC Reset Counters
        static uint16_t msgid   = PAYUZUC_CMD_ID;
    ...
    case 210: { // PAYUZUC Memory Status (no-arg)
        static uint16_t msgid   = PAYUZUC_CMD_ID;
    ...
    case 214: { // PAYUZUC Read Register
        static uint16_t msgid   = PAYUZUC_CMD_ID;
    ...
    case 215: { // PAYUZUC Write Register
        static uint16_t msgid   = PAYUZUC_CMD_ID;
    ...
    case 218: { // PAYUZUC Download All Child (CC 23, payload same as DownloadAll)
        static uint16_t msgid   = PAYUZUC_CMD_ID;
...
{   // Common type cmd
    ...
    // KISSCAM
    snprintf(Templabels[206], 64, "PAYUZUC NoOp");
    snprintf(Templabels[207], 64, "PAYUZUC Reset Counters");
    snprintf(Templabels[208], 64, "PAYUZUC Ping");
    snprintf(Templabels[209], 64, "PAYUZUC Set Mode");
    snprintf(Templabels[210], 64, "PAYUZUC Memory Status");
    snprintf(Templabels[211], 64, "PAYUZUC Set Exposure");
    snprintf(Templabels[212], 64, "PAYUZUC Capture");
    snprintf(Templabels[213], 64, "PAYUZUC Download");
    snprintf(Templabels[214], 64, "PAYUZUC Read Register");
    snprintf(Templabels[215], 64, "PAYUZUC Write Register");
    snprintf(Templabels[216], 64, "PAYUZUC Download All");
    snprintf(Templabels[217], 64, "PAYUZUC Mosaic");
    snprintf(Templabels[218], 64, "PAYUZUC Download All Child");
