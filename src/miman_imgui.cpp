...
void ImGui_BeaconWindow(float fontscale)
{
    ...
    if(ImGui::BeginTabItem("Beacon"))
    {
        if (ImGui::BeginTable("##BeaconTables", 2, ImGuiTableFlags_SizingStretchSame | ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable))
        {
            ...
            ImGui::Text("%"PRIu8, beacon->CurrentMode);
            StateWindowColumnManager("Current SubMode");
            ImGui::Text("%"PRIu8, beacon->CurrentSubmode);
            StateWindowColumnManager("Previous Mode");
            ImGui::Text("%"PRIu8, beacon->PrevioudMode);
            StateWindowColumnManager("Previous SubMode");
            ImGui::Text("%"PRIu8, beacon->PreviousSubmode);
            StateWindowColumnManager("Current Mode Flag");
            ImGui::Text("%"PRIu8, beacon->CurrentModeFlag);
            StateWindowColumnManager("Previous Mode Flag");
            ImGui::Text("%"PRIu8, beacon->PreviousModeFlag);
            StateWindowColumnManager("Application Run Status");
            ImGui::Text("%"PRIu32, beacon->ApplicationRunStatus);
            StateWindowColumnManager("Satellite Time");
            ImGui::Text("%"PRIu32, beacon->SatelliteTime);
            //UHF ANT
            StateWindowColumnManager("UHF Antenna Deployment State");
            ImGui::Text("%"PRIu16, beacon->DeployState_UANT);
            //UTRX
            StateWindowColumnManager("Rx Frequency");
            ImGui::Text("%"PRIu32, beacon->rxfreq);
            StateWindowColumnManager("Tx Frequency");
            ImGui::Text("%"PRIu32, beacon->txfreq);
            StateWindowColumnManager("Last RSSI");
            ImGui::Text("%"PRId16, beacon->LastRssi);
            StateWindowColumnManager("Total Rx Bytes");
            ImGui::Text("%"PRIu32, beacon->TotRxBytes);
            StateWindowColumnManager("Status Configuration");
            ImGui::Text("%"PRIu8, beacon->StatusConfiguration);
            //PCDU P60 Dock
            StateWindowColumnManager("PCDU Dock Output Enable Status");
            ImGui::Text("%"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8, beacon->out_en_dock[0], beacon->out_en_dock[1], beacon->out_en_dock[2], beacon->out_en_dock[3], beacon->out_en_dock[4], beacon->out_en_dock[5], beacon->out_en_dock[6]);
            StateWindowColumnManager("PCDU Dock Temperature");
            ImGui::Text("%"PRIu16" %"PRIu16, beacon->temp_dock[0], beacon->temp_dock[1]);
            StateWindowColumnManager("PCDU Boot Cause");
            ImGui::Text("%"PRIu32, beacon->bootcause);
            StateWindowColumnManager("PCDU Boot Count");
            ImGui::Text("%"PRIu32, beacon->bootcnt);
            StateWindowColumnManager("PCDU Uptime");
            ImGui::Text("%"PRIu32, beacon->uptime);
            StateWindowColumnManager("PCDU Reset Cause");
            ImGui::Text("%"PRIu16, beacon->resetcause);
            StateWindowColumnManager("Battery Mode");
            ImGui::Text("%"PRIu8, beacon->batt_mode);
            StateWindowColumnManager("Heater Enabled Status");
            ImGui::Text("%"PRIu8, beacon->heater_on);
            StateWindowColumnManager("Latchup Protection Count");
            ImGui::Text("%"PRId16" %"PRId16" %"PRId16" %"PRId16" %"PRId16" %"PRId16" %"PRId16, beacon->latchup_dock[0], beacon->latchup_dock[1], beacon->latchup_dock[2], beacon->latchup_dock[3], beacon->latchup_dock[4], beacon->latchup_dock[5], beacon->latchup_dock[6]);
            StateWindowColumnManager("PCDU Dock VBAT Voltage");
            ImGui::Text("%"PRIu16, beacon->vbat_v);
            StateWindowColumnManager("Battery Voltage");
            ImGui::Text("%"PRIu16, beacon->batt_v);
            StateWindowColumnManager("Battery Temperature");
            ImGui::Text("%"PRId16, beacon->batt_temp[0], beacon->batt_temp[1]);
            StateWindowColumnManager("PCDU Device Status");
            ImGui::Text("%"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8, beacon->device_status[0], beacon->device_status[1], beacon->device_status[2], beacon->device_status[3], beacon->device_status[4], beacon->device_status[5], beacon->device_status[6], beacon->device_status[7]);
            StateWindowColumnManager("Ground WDT Reboot Count");
            ImGui::Text("%"PRIu32, beacon->wdt_cnt_gnd);
            StateWindowColumnManager("Ground WDT Remaining Seconds");
            ImGui::Text("%"PRIu32, beacon->wdt_gnd_left);
            StateWindowColumnManager("Battery Charge Current");
            ImGui::Text("%"PRId16, beacon->batt_chrg);
            StateWindowColumnManager("Battery Discharge Current");
            ImGui::Text("%"PRId16, beacon->batt_dischrg);
            //PCDU PDU-200
            StateWindowColumnManager("PDU VBAT Voltage");
            ImGui::Text("%"PRId16, beacon->vbat);
            StateWindowColumnManager("PDU Output Enable Status");
            ImGui::Text("%"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8" %"PRIu8, beacon->out_en_pdu[0], beacon->out_en_pdu[1], beacon->out_en_pdu[2], beacon->out_en_pdu[3], beacon->out_en_pdu[4], beacon->out_en_pdu[5]);
            StateWindowColumnManager("PDU Latchup Protection Count");
            ImGui::Text("%"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16, beacon->latchup_pdu[0], beacon->latchup_pdu[1], beacon->latchup_pdu[2], beacon->latchup_pdu[3], beacon->latchup_pdu[4], beacon->latchup_pdu[5]);
            StateWindowColumnManager("PDU Output Converter Voltage");
            ImGui::Text("%"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16, beacon->out_voltage[0], beacon->out_voltage[1], beacon->out_voltage[2], beacon->out_voltage[3], beacon->out_voltage[4], beacon->out_voltage[5]);
            //PCDU ACU-200
            StateWindowColumnManager("ACU Charging Current");
            ImGui::Text("%"PRId16" %"PRId16" %"PRId16" %"PRId16, beacon->c_in[0], beacon->c_in[1], beacon->c_in[2], beacon->c_in[3]);
            StateWindowColumnManager("ACU Charging Voltage");
            ImGui::Text("%"PRIu16" %"PRIu16" %"PRIu16" %"PRIu16, beacon->v_in[0], beacon->v_in[1], beacon->v_in[2], beacon->v_in[3]);
            //ADCS
            StateWindowColumnManager("RWL0 Power State");
            ImGui::Text("%"PRIu8, beacon->RWL0_PowerState);
            StateWindowColumnManager("RWL1 Power State");
            ImGui::Text("%"PRIu8, beacon->RWL1_PowerState);
            StateWindowColumnManager("RWL2 Power State");
            ImGui::Text("%"PRIu8, beacon->RWL2_PowerState);
            StateWindowColumnManager("MAG0 Power State");
            ImGui::Text("%"PRIu8, beacon->MAG0_PowerState);
            StateWindowColumnManager("GYRO0 Power State");
            ImGui::Text("%"PRIu8, beacon->GYRO0_PowerState);
            StateWindowColumnManager("FSS0 Power State");
            ImGui::Text("%"PRIu8, beacon->FSS0_PowerState);
            StateWindowColumnManager("HSS0 Power State");
            ImGui::Text("%"PRIu8, beacon->HSS0_PowerState);
            StateWindowColumnManager("ADCS Control Mode");
            ImGui::Text("%"PRIu8, beacon->Control_Mode);
            StateWindowColumnManager("Magnetic Control Timeout");
            ImGui::Text("%"PRIu16, beacon->Mag_Control_Timeout);
            StateWindowColumnManager("GYRO Calibrated Rate X");
            ImGui::Text("0x%x", beacon->GYRO_Calib_rate_X);
            StateWindowColumnManager("GYRO Calibrated Rate Y");
            ImGui::Text("0x%x", beacon->GYRO_Calib_rate_Y);
            StateWindowColumnManager("GYRO Calibrated Rate Z");
            ImGui::Text("0x%x", beacon->GYRO_Calib_rate_Z);
            //STX
            StateWindowColumnManager("STx Status");
            ImGui::Text("%"PRIu8, beacon->Status);
            StateWindowColumnManager("STx Temperature");
            ImGui::Text("%"PRId16, beacon->Board_Temperature);
            StateWindowColumnManager("STx Current");
            ImGui::Text("%"PRId16, beacon->Battery_Current);
            StateWindowColumnManager("STx Voltage");
            ImGui::Text("%"PRId16, beacon->Battery_Voltage);
            //PAYC
            StateWindowColumnManager("PAYC Temperature");
            ImGui::Text("%"PRIu16, beacon->temp_PAYC);
            StateWindowColumnManager("PAYC Current");
            ImGui::Text("%"PRIu16, beacon->icore);
            //PAYR
            StateWindowColumnManager("PAYR Deployment Status");
            ImGui::Text("%"PRIu8, beacon->DeployStatus_PAYR);            
            //PAYS
            StateWindowColumnManager("PAYS State");
            ImGui::Text("%"PRIu8, beacon->PAYS_State);
            StateWindowColumnManager("PAYS Temperature Sign");
            ImGui::Text("%"PRIu8, beacon->PAYS_Sign);
            StateWindowColumnManager("PAYS Temperature");
            ImGui::Text("%"PRIu8, beacon->PAYS_Temp);
        }  
        ImGui::EndTable();
        ImGui::EndTabItem();
            ...
            case 0: {  //no arg
                ...
                if(ImGui::Button("Generate CMD")) {
                    if(State.Scheduled)
                    {
                        ...
                        sch_cmd_t* schcmd;
                        schcmd = (sch_cmd_t *)malloc(5+2+2+4+CFE_SB_CMD_HDR_SIZE); // 8 is for data
            ...
            case 4:  {// u64
                ...
                if(ImGui::Button("Generate CMD")) {
                    if(State.Scheduled)
                    {
                        ...
                        sch_cmd_t* schcmd;
                        schcmd = (sch_cmd_t *)malloc(5+2+2+4+CFE_SB_CMD_HDR_SIZE+sizeof(uint64_t)); // 8 is for data
                        schcmd->TestPacket->Identifier = HVD_TEST;
                        schcmd->TestPacket->PacketType = MIM_PT_TMTC_TEST;
