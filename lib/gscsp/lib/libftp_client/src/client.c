                         ...
                         gs_ftp_info_callback_t info_callback, void * info_data)
{
    ...
    int res_length = 0;
    if (res_length = csp_transaction_persistent(state.conn, gs_ftp_get_timeout(settings) + rx_delay_ms(rep_length, 3), &req, req_length, &rep, rep_length) != rep_length) {
...
static gs_error_t ftp_data(int count)
{
    ...
    uint32_t rdp_opt_window_size;
    uint32_t rdp_opt_conn_timeout_ms;
    uint32_t rdp_opt_packet_timeout_ms;
    uint32_t rdp_opt_delayed_acks;
    uint32_t rdp_opt_ack_timeout;
    uint32_t rdp_opt_ack_delay_count;

    // uint16_t destination = state.conn->idout.dst;
    ...
    for (i = 0; i < state.last_entries; i++) {
        ...
        for (j = 0; j < n->count; j++) {
            ...
            /* Read chunk */
            if ((unsigned int) ftell(state.fp) != streamdata.data.chunk * state.chunk_size)
                fseek(state.fp, streamdata.data.chunk * state.chunk_size, SEEK_SET);
            ret = fread(streamdata.data.bytes, 1, state.chunk_size, state.fp);
            ...
            if (ret < state.chunk_size) {
                memset(&streamdata.data.bytes[ret], 0, state.chunk_size - ret);
            }
            ...
            /* Send data */
            int length = sizeof(streamdata.type) + sizeof(uint32_t) + state.chunk_size;
...
void ftp_done(unsigned int remove_map)
{
    ...
    memset(&state, 0, sizeof(gs_ftp_state_t));
