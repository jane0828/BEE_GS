...
static uint32_t gs_ftp_get_csp_opts(const gs_ftp_settings_t * settings, uint32_t default_opts)
{
    return (settings && settings->csp_opts) ? settings->csp_opts : default_opts;
}
                         ...
                         gs_ftp_info_callback_t info_callback, void * info_data)
{
    ...
    uint32_t csp_opts = gs_ftp_get_csp_opts(settings, CSP_O_RDP | CSP_O_CRC32);
    printf("[FTP-LIB] upload csp_connect: host=%u port=%u timeout=%u opts=0x%x rdp=%s\n",
           settings->host,
           gs_ftp_get_csp_port(settings),
           gs_ftp_get_timeout(settings),
           csp_opts,
           (csp_opts & CSP_O_RDP) ? "ON" : "OFF");
    fflush(stdout);
    state.conn = csp_connect(CSP_PRIO_NORM, settings->host, gs_ftp_get_csp_port(settings), gs_ftp_get_timeout(settings),
                             csp_opts);
    if (state.conn == NULL) {
        printf("[FTP-LIB] upload csp_connect failed: host=%u port=%u timeout=%u opts=0x%x rdp=%s return=GS_ERROR_IO%s\n",
               settings->host,
               gs_ftp_get_csp_port(settings),
               gs_ftp_get_timeout(settings),
               csp_opts,
               (csp_opts & CSP_O_RDP) ? "ON" : "OFF",
               (csp_opts & CSP_O_RDP) ? " reason=RDP handshake/connect failed before FTP transaction. Target may be absent or no SYN-ACK received." : "");
        fflush(stdout);
        ftp_done(&state, 0);
        return GS_ERROR_IO;
    }
                           ...
                           gs_ftp_info_callback_t info_callback, void * info_data)
{
    ...
    int req_length = sizeof(req.type) + sizeof(req.down);
    uint32_t csp_opts = gs_ftp_get_csp_opts(settings, CSP_O_RDP | CSP_O_CRC32);
    printf("[FTP-LIB] download csp_connect: host=%u port=%u timeout=%u opts=0x%x rdp=%s\n",
           settings->host,
           gs_ftp_get_csp_port(settings),
           gs_ftp_get_timeout(settings),
           csp_opts,
    ...
    state.conn = csp_connect(CSP_PRIO_NORM, settings->host, gs_ftp_get_csp_port(settings), gs_ftp_get_timeout(settings),
                              csp_opts);
    if (state.conn == NULL) {
        printf("[FTP-LIB] download csp_connect failed: host=%u port=%u timeout=%u opts=0x%x rdp=%s return=GS_ERROR_IO%s\n",
               settings->host,
               gs_ftp_get_csp_port(settings),
               gs_ftp_get_timeout(settings),
               csp_opts,
